...
<xsl:stylesheet 
  ...
  <xsl:template match="html:tbody | html:thead | html:tfoot | html:table[html:tr]" mode="htmltable:normalize-rowspans">
    <xsl:copy>
      <xsl:copy-of select="@*" />
      <xsl:apply-templates select="* except html:tr" mode="#current" />
      ...
      <xsl:apply-templates select="html:tr[2]" mode="htmltable:normalize-rowspans">
        <xsl:with-param name="previousRow" select="html:tr[1]" />
        <xsl:with-param name="rownum" select="2" />
      </xsl:apply-templates>
    </xsl:copy>
  </xsl:template>

  <xsl:template match="html:td | html:th" mode="htmltable:normalize-colspans">
      ...
      <xsl:for-each select="$this"><!-- this akward for-each is needed in order for the template to stay namespace-agnostic -->
        <xsl:copy>
          ...
          <xsl:if test="$count eq 1">
            <xsl:apply-templates select="@html:width" mode="#current" />
          </xsl:if>
  ...
  <xsl:template match="*[html:tr[*/@data-twips-width]]" mode="htmltable:normalize-final">
    <xsl:copy copy-namespaces="no">
      <xsl:apply-templates select="@*" mode="#current" />
      <xsl:element name="colgroup" namespace="{namespace-uri(.)}">
        <xsl:variable name="context" select="." as="element(*)" />
        <xsl:for-each select="1 to count(html:tr[1]/*)">
          <xsl:element name="col" namespace="{namespace-uri($context)}">
            <xsl:copy-of select="($context/html:tr/*[position() eq current()]
                                                 [not(@colspan &gt; 1)]
  ...
  <xsl:template match="html:tr" mode="htmltable:normalize-rowspans">
    <xsl:param name="previousRow" as="element()?" />
    <xsl:param name="rownum" as="xs:integer?" />
    <xsl:if test="count($previousRow) eq 0">
      <xsl:message>html-tables-normalize.xsl, mode htmltable:normalize-rowspans, matching html:tr: previousRow is unexpectedly empty. Is the tr in a table/tgroup? Parent: <xsl:value-of select="name(..)"/>
      </xsl:message>
    </xsl:if>
    ...
    <xsl:apply-templates select="following-sibling::html:tr[1]" mode="htmltable:normalize-rowspans">
      <xsl:with-param name="previousRow" select="$newRow" />
      <xsl:with-param name="rownum" select="$rownum + 1" />
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="html:td | html:th" mode="htmltable:normalize-rowspans">
    <xsl:param name="rownum" as="xs:integer?" />
    <xsl:choose>
